...
class GuardrailsRegistry {
  ...
  constructor() {
    this.auditLogPath = path.join(process.cwd(), 'data', 'audit', 'guardrails.log');
  ...
  private registerDefaultActions() {
    ...
    console.log('[Guardrails] Registered', this.actions.size, 'critical actions');
  }
   ...
   */
  isAllowedUser(actionId: string, userId: string, userRoles: string[]): boolean {
    const action = this.actions.get(actionId);
    if (!action) return false;

    // Check explicit user allowlist
    if (action.allowedUsers.length > 0 && !action.allowedUsers.includes(userId)) {
      return false;
    }

    // Check role requirements
    if (action.allowedRoles.length > 0) {
      const hasRequiredRole = action.allowedRoles.some(role => userRoles.includes(role));
      if (!hasRequiredRole) {
        return false;
      }
    }
   ...
   * Check rate limits
   */
  checkRateLimits(actionId: string, userId: string): { allowed: boolean; reason?: string } {
    const action = this.actions.get(actionId);
    if (!action) {
    ...
    const recentHour = userHistory.filter(req =>
      req.actionId === actionId &&
      new Date(req.timestamp).getTime() > oneHourAgo
    );

    const recentDay = userHistory.filter(req =>
      req.actionId === actionId &&
    ...
    if (recentHour.length >= action.rateLimits.maxPerHour) {
      return { allowed: false, reason: `Hourly rate limit exceeded (${recentHour.length}/${action.rateLimits.maxPerHour})` };
    }

    if (recentDay.length >= action.rateLimits.maxPerDay) {
      return { allowed: false, reason: `Daily rate limit exceeded (${recentDay.length}/${action.rateLimits.maxPerDay})` };
    }
   ...
   */
  checkCooldown(actionId: string): { allowed: boolean; remainingTime?: number } {
    const action = this.actions.get(actionId);
    if (!action) {
  ...
  validateActionRequest(
    ...
    if (!cooldownCheck.allowed) {
      ...
      return { allowed: false, reason: `Cooldown period active. ${remainingSeconds}s remaining` };
    }

    // Check environment restrictions
    if (action.riskLevel === RiskLevel.CRITICAL && environment === 'production') {
   ...
   */
  rejectAction(requestId: string, approverId: string, reason?: string): boolean {
    for (const userRequests of this.actionHistory.values()) {
      const request = userRequests.find(r => r.id === requestId);
      if (request) {
   ...
   */
  markAsExecuted(requestId: string, success: boolean, error?: string, data?: Record<string, unknown>): void {
    for (const userRequests of this.actionHistory.values()) {
      const request = userRequests.find(r => r.id === requestId);
      if (request) {
   ...
   */
  private logToAudit(request: ActionRequest, status: string = 'CREATED', additionalInfo?: string): void {
    const logEntry = {
  ...
  getStats(): {
    ...
    for (const [userId, userRequests] of this.actionHistory.entries()) {
      ...
      for (const request of userRequests) {
        ...
        if (request.status === 'executed' && new Date(request.timestamp).getTime() > oneDayAgo) {
          executionsLast24h++;
        }
