...
export async function googleOAuthRoutes(fastify: FastifyInstance) {
    ...
    try {
      ...
      if (error) {
        return {
          ...
          message: 'You denied access to Google Calendar. Please try again and grant permission.'
        };
      }
   ...
   */
  fastify.post('/auth/google/logout', async (request, reply) => {
    try {
      // Clear tokens
      const { googleOAuthManager } = await import('../integrations/google/oauth-manager.js');
      googleOAuthManager.clearTokens();

      return {
        success: true,
        message: 'Google Calendar authorization cleared. You can authorize again if needed.'
      };
    } catch (error) {
      reply.code(500);
      return {
