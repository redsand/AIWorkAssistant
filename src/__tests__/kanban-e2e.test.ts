...
describe('Kanban Smoke E2E', () => {
  ...
  beforeAll(async () => {
    ...
    const { githubClient } = await import('../integrations/github/github-client.js');
    ...
    const { kanbanRoutes } = await import('../routes/kanban.js');
    app = Fastify();
  ...
  it('drives the full happy path: start agent, receive SSE steps, complete, cleanup worktree', async () => {
    ...
    const { runAgent } = await import('../autonomous-loop/agent-runner.js');
