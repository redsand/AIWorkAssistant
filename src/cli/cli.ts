...
const profileCmd = program
  .command("profile")
  .description("Manage isolated agent profiles (separate SOUL.md, MEMORY.md, skills)");
...
profileCmd
  ...
  .action((name: string, opts: { clone?: string }) => {
    try {
      ...
      const profile = pm.create(name, opts.clone ? { clone: opts.clone } : undefined);
      console.log(`Created profile '${profile.name}' at ${profile.path}`);
      if (opts.clone) {
        console.log(`  Cloned config and .env from '${opts.clone}'`);
      }
    } catch (err) {
      console.error("Failed to create profile:", err instanceof Error ? err.message : err);
      process.exit(1);
    }
  });
...
profileCmd
  ...
  .action((name: string) => {
    try {
      ...
      console.error("Failed to switch profile:", err instanceof Error ? err.message : err);
      process.exit(1);
    }
  });
