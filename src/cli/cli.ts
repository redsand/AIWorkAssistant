 ...
 * Chat with the agent
 */
async function chatWithAgent(message: string, mode: 'productivity' | 'engineering' = 'productivity'): Promise<void> {
  try {
 ...
 */
async function streamChatWithAgent(message: string, mode: 'productivity' | 'engineering' = 'productivity'): Promise<void> {
  try {
    console.log('🤖 Agent:', message);
...
async function getApprovals(): Promise<void> {
  try {
    ...
    approvals.forEach((approval: any, index: number) => {
      ...
      console.log(`     Requested: ${new Date(approval.requestedAt).toLocaleString()}`);
      console.log(`     ID: ${approval.id}`);
      console.log('');
    });
    console.log(`💡 Approve: curl -X POST ${API_BASE_URL}/approvals/${approvals[0].id}/approve -H "Content-Type: application/json" -d '{"userId":"cli-user"}'`);
    console.log(`   Reject: curl -X POST ${API_BASE_URL}/approvals/${approvals[0].id}/reject -H "Content-Type: application/json" -d '{"userId":"cli-user"}'`);
...
async function checkHealth(): Promise<void> {
  try {
    ...
    if (health.opencode) {
      ...
      console.log(`   Configured: ${health.opencode.configured ? 'Yes' : 'No'}`);
...
async function generateEngineeringStrategy(idea: string): Promise<void> {
  await chatWithAgent(`I want to build: ${idea}

Please help me design this properly by starting with workflow analysis.`, 'engineering');
}
...
async function generateWorkflowBrief(idea: string): Promise<void> {
  await chatWithAgent(`Generate a workflow brief for: ${idea}
...
async function generateArchitecture(idea: string): Promise<void> {
  await chatWithAgent(`Generate an architecture proposal for: ${idea}
...
- Data model
- API design
- Security considerations`, 'engineering');
}
...
async function generateImplementationPlan(idea: string): Promise<void> {
  await chatWithAgent(`Generate an implementation plan for: ${idea}
...
program
  .name('openclaw-agent')
  .description('OpenClaw Agent CLI - Personal productivity and engineering copilot')
  .version('0.1.0');
...
program
  ...
  .option('-m, --mode <mode>', 'Productivity or engineering mode', 'productivity')
...
program
  ...
  .action(async (options, duration) => {
    await chatWithAgent(`Create a ${duration} focus block for deep work`, 'productivity');
  });
  ...
  .action(async (options, query) => {
    try {
      ...
      results.forEach((memory: any, index: number) => {
        ...
        console.log(`   ${new Date(memory.startDate).toLocaleDateString()} - ${new Date(memory.endDate).toLocaleDateString()}`);
