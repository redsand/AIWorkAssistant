/**
 * Discord Bot Integration for OpenClaw Agent
 * Enables chatting with the AI agent through Discord
 */

import { Client, GatewayIntentBits, Message, SlashCommandBuilder, REST, Routes } from 'discord.js';
...
class DiscordAgentBot {
  ...
  private async registerCommands(): Promise<void> {
    ...
    try {
      ...
      if (this.config.guildId) {
        ...
        console.log('[DiscordBot] Registered global commands (may take up to 1 hour to propagate)');
      }
    } catch (error) {
  ...
  private async onMessage(message: Message): Promise<void> {
    ...
    // Process the message
    const content = message.content.replace(new RegExp(`<@!?${this.client.user!.id}>`), '').trim();
  ...
  private async handleChatCommand(interaction: any): Promise<void> {
    ...
    const mode = interaction.options.getString('mode') as 'productivity' | 'engineering' || 'productivity';
  ...
  private async handlePlanCommand(interaction: any): Promise<void> {
    ...
    const response = await this.sendToAgent(message, interaction.user.id, 'productivity');
    ...
    switch (action) {
      case 'list': {
        ...
        if (roadmaps.length === 0) {
          await interaction.editReply('No roadmaps found. Create one with /roadmap create');
          return;
        }
      ...
      case 'create':
        await interaction.reply({
          content: 'To create a roadmap, please provide:\n• Name\n• Type (client/internal)\n• Template (optional)',
          ephemeral: true
        });
        break;
    }
    ...
    switch (action) {
      ...
      case 'end':
        if (session && session.sessionId) {
          try {
            await axios.post(`${this.apiBaseUrl}/chat/sessions/${session.sessionId}/end`);
      ...
      case 'info':
        if (session && session.sessionId) {
          try {
            const response = await axios.get(`${this.apiBaseUrl}/chat/sessions/${session.sessionId}`);
          ...
          await interaction.reply('ℹ️ No active session. Start one with /session start');
        }
        break;
    }
  }
  ...
  private async handleMemoryCommand(interaction: any): Promise<void> {
    ...
    try {
      const response = await axios.get(`${this.apiBaseUrl}/chat/memory/search`, {
        params: {
          userId: interaction.user.id,
          query,
          limit: 5,
...
**How to Use:**
1. Mention the bot in any message: \`@OpenClaw your message\`
...
Just start chatting with me! 🚀
   ...
   */
  private async sendToAgent(message: string, userId: string, mode: 'productivity' | 'engineering'): Promise<string> {
    try {
   ...
   */
  private async sendLongReply(message: Message | any, content: string): Promise<void> {
