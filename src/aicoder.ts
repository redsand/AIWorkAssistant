    ...
    try {
      const resp = await axios.post<{ success: boolean; mrIid?: number; url?: string; error?: string }>(
        `${cfg.apiUrl}/api/autonomous-loop/mr`,
        {
          ...
          description: item.url ? `Closes ${item.url}\n\n_Generated by AiRemoteCoder autonomous agent._` : "_Generated by AiRemoteCoder autonomous agent._",
...
function recoverFromRebase(cwd: string): boolean {
  ...
  // Step 4: Manual cleanup — remove rebase state directories and soft-reset
  runLogger.logGit("WARN", "Manual rebase cleanup — removing .git/rebase-merge and .git/rebase-apply");
  const gitDir = path.join(cwd, ".git");
  ...
  // Read the original HEAD from the rebase state before cleaning up
  ...
  if (current === branchName) {
    ...
    if (!gitRun(["rebase", getBaseBranch()], WORKSPACE)) {
      ...
      if (!gitRun(["rebase", "--abort"], WORKSPACE)) {
        runLogger.logGit("WARN", "git rebase --abort failed — attempting recovery");
...
// Recover from any mid-rebase state left by a previous run
if (isRebaseInProgress(WORKSPACE)) {
  runLogger.logConfig("Mid-rebase state detected from previous run — recovering");
  recoverFromRebase(WORKSPACE);
}
