...
async function testGoogleCalendar() {
  ...
  console.log(
    `Client ID: ${env.GOOGLE_CALENDAR_CLIENT_ID ? "✅ Set" : "❌ Missing"}`,
  );
  console.log(
    `Client Secret: ${env.GOOGLE_CALENDAR_CLIENT_SECRET ? "✅ Set" : "❌ Missing"}`,
  );
  ...
  if (!googleCalendarClient.isConfigured()) {
    ...
    console.error(
      "5. Add http://localhost:3050/auth/google/callback to authorized redirect URIs",
    );
  ...
  try {
    ...
    const testEvent = await calendarService.createEvent(
      {
        summary: "🧪 AI Assistant Test Event",
        description: "This is a test event created by AI Assistant Agent",
    ...
    console.log(
      "🔗 Your calendar is now fully integrated with AI Assistant Agent.",
    );
  } catch (error) {
    console.error("\n❌ Test failed:", error);
    process.exit(1);
