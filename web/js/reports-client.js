 ...
 * report download endpoint gets fetched with auth and saved as a blob —
 * regardless of whether it came from the slash command, the model's
 * response text, or a manifest link.
 */
export function installReportDownloadInterceptor() {
  document.addEventListener("click", async (evt) => {
    ...
    if (!isReportDownloadUrl(href)) return;
    evt.preventDefault();
    evt.stopPropagation();
    ...
    try {
      ...
      a.href = url;
      a.download = inferFilename(href, res);
...
function inferFilename(href, res) {
...
// Exported for tests
export const __test = { parseReportCommand, isReportDownloadUrl, inferFilename };
