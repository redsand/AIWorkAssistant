...
describe("getPlatformForToolName", () => {
  ...
  it("maps tools.discover to cross-platform", () => {
    expect(getPlatformForToolName("tools.discover")).toBe("cross-platform");
  });
...
describe("getToolsByPlatform", () => {
  ...
  it("returns cross-platform tools for cross-platform platform", () => {
    ...
    for (const tool of tools) {
      ...
      expect(["productivity", "todo", "knowledge", "system", "agent", "workflow", "roadmap", "engineering", "codebase", "graph", "git", "discover", "work_items", "cto", "personal_os", "product", "memory", "hawk_ir", "code_review", "skill", "soul", "session", "cron", "tools", "gateway", "profile", "update_entity_md"]).toContain(prefix);
    }
  });
});
