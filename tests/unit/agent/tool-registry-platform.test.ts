...
describe("getToolsByPlatform", () => {
  ...
  it("returns cross-platform tools for cross-platform platform", () => {
    ...
    for (const tool of tools) {
      ...
      expect(["productivity", "todo", "knowledge", "system", "agent", "workflow", "roadmap", "engineering", "codebase", "graph", "git", "discover", "work_items", "cto", "personal_os", "product", "memory", "hawk_ir", "code_review", "skill", "soul"]).toContain(prefix);
    }
  });
});
