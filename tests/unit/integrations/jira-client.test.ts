...
describe('Jira Client', () => {
  beforeAll(() => {
    ...
    if (!jiraClient.isConfigured()) {
      console.warn('Jira credentials not configured - skipping integration tests');
    }
  });
  ...
  describe('Issue Operations', () => {
    ...
    it('should get available projects', async () => {
      ...
      if (projects.length > 0) {
        console.log('Sample projects:', projects.slice(0, 3).map(p => `${p.key}: ${p.name}`));
      }
    }, 15000);
    ...
    it('should get assigned issues', async () => {
      ...
      if (issues.length > 0) {
        console.log('Sample issues:', issues.slice(0, 3).map(i => `${i.key}: ${i.fields.summary}`));
      }
    }, 30000);
    ...
    it('should search issues with JQL', async () => {
      ...
      // Search for recent issues updated in last 24 hours
